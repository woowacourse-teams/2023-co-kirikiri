...
import static co.kirikiri.goalroom.service.dto.GoalRoomMemberSortTypeDto.ACCOMPLISHMENT_RATE;
...
class GoalRoomReadApiTest extends ControllerTestHelper {
                ...
                .andDo(
                        documentationResultHandler.document(
                                ...
                                responseFields(
                                        ...
                                        fieldWithPath("checkFeeds[0].createdAt").description("인증 피드 등록 날짜")
                                )))
                .andReturn().getResponse()
                .getContentAsString();
    ...
    private MemberGoalRoomResponse 사용자_골룸_조회_응답을_생성한다() {
        return new MemberGoalRoomResponse("골룸 이름", "RUNNING", 1L,
                ...
                List.of(new DashBoardToDoResponse(1L, "첫 번째 할일",
                        LocalDate.of(2023, 1, 15), LocalDate.of(2023, 1, 31),
                        new DashBoardToDoCheckResponse(false))),
                List.of(new DashBoardCheckFeedResponse(1L, "imageUrl1", "인증 피드 설명 1", LocalDate.now()),
                        new DashBoardCheckFeedResponse(2L, "imageUrl2", "인증 피드 설명 2", LocalDate.now()),
                        new DashBoardCheckFeedResponse(3L, "imageUrl3", "인증 피드 설명 3", LocalDate.now()),
                        new DashBoardCheckFeedResponse(4L, "imageUrl4", "인증 피드 설명 4", LocalDate.now())));
