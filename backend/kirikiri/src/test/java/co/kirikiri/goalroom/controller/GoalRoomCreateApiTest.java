package co.kirikiri.goalroom.controller;
...
import co.kirikiri.goalroom.service.GoalRoomCreateService;
import co.kirikiri.goalroom.service.GoalRoomReadService;
...
class GoalRoomCreateApiTest extends ControllerTestHelper {
    ...
    void 골룸_생성_시_요청에_빈값이_있을_경우() throws Exception {
        ...
        final GoalRoomCreateRequest request = new GoalRoomCreateRequest(null, null,
                null, new ArrayList<>(List.of(new GoalRoomRoadmapNodeRequest(null, null, null, null))));
        final String jsonRequest = objectMapper.writeValueAsString(request);
    ...
    void 골룸_생성_시_골룸_노드의_시작_날짜보다_종료_날짜가_빠른_경우() throws Exception {
        //given
        final GoalRoomCreateRequest request = new GoalRoomCreateRequest(1L, "name",
                20, new ArrayList<>(List.of(new GoalRoomRoadmapNodeRequest(1L, 10, TEN_DAY_LATER, TODAY))));
        final String jsonRequest = objectMapper.writeValueAsString(request);
    ...
    void 골룸_생성_시_골룸_노드의_인증_횟수가_0보다_작을_경우() throws Exception {
        //given
        final GoalRoomCreateRequest request = new GoalRoomCreateRequest(1L, "name",
                20, new ArrayList<>(List.of(new GoalRoomRoadmapNodeRequest(1L, 0, TODAY, TEN_DAY_LATER))));
        final String jsonRequest = objectMapper.writeValueAsString(request);
    ...
    void 골룸_생성_시_골룸_노드의_인증_횟수가_기간보다_클_경우() throws Exception {
        //given
        final GoalRoomCreateRequest request = new GoalRoomCreateRequest(1L, "name",
                20, new ArrayList<>(List.of(new GoalRoomRoadmapNodeRequest(1L, 11, TODAY, TEN_DAY_LATER))));
        final String jsonRequest = objectMapper.writeValueAsString(request);
    ...
    @Test
    void 골룸을_시작한다() throws Exception {
        // given
        doNothing().when(goalRoomCreateService)
                .startGoalRoom(anyString(), anyLong());

        // when
        mockMvc.perform(post(API_PREFIX + "/goal-rooms/{goalRoomId}/start", 1L)
                        .header(AUTHORIZATION, "Bearer <AccessToken>")
                        .contextPath(API_PREFIX))
                .andDo(documentationResultHandler.document(
                        requestHeaders(
    ...
    @Test
    void 골룸_시작시_존재하지_않는_사용자면_예외가_발생한다() throws Exception {
        // given
        doThrow(new NotFoundException("존재하지 않는 회원입니다."))
                .when(goalRoomCreateService)
                .startGoalRoom(anyString(), anyLong());
        ...
        // when
        final MvcResult mvcResult = mockMvc.perform(post(API_PREFIX + "/goal-rooms/{goalRoomId}/start", 1L)
    ...
    @Test
    void 골룸_시작시_존재하지_않는_골룸이면_예외가_발생한다() throws Exception {
        // given
        doThrow(new NotFoundException("존재하지 않는 골룸입니다. goalRoomId = 1"))
                .when(goalRoomCreateService)
                .startGoalRoom(anyString(), anyLong());
    ...
    @Test
    void 골룸을_시작하는_사용자가_골룸의_리더가_아니면_예외가_발생한다() throws Exception {
        // given
        doThrow(new NotFoundException("골룸의 리더만 골룸을 시작할 수 있습니다."))
                .when(goalRoomCreateService)
                .startGoalRoom(anyString(), anyLong());
        ...
        assertThat(response).isEqualTo(new ErrorResponse("골룸의 리더만 골룸을 시작할 수 있습니다."));
    }

    @Test
    void 골룸_시작시_골룸의_시작날짜가_미래라면_예외가_발생한다() throws Exception {
        // given
        doThrow(new NotFoundException("골룸의 시작 날짜가 되지 않았습니다."))
                .when(goalRoomCreateService)
                .startGoalRoom(anyString(), anyLong());
