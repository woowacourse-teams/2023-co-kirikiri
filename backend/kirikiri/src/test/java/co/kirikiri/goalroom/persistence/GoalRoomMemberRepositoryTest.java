...
class GoalRoomMemberRepositoryTest {
    ...
    void 골룸과_사용자_아이디로_골룸_사용자_목록을_조회한다() {
        ...
        // when
        final Optional<GoalRoomMember> findGoalRoomMember = goalRoomMemberRepository.findByGoalRoomAndMemberId(
    ...
    void 골룸으로_사용자_목록과_멤버를_함께_조회한다() {
        ...
        final GoalRoomMember goalRoomMember1 = new GoalRoomMember(GoalRoomRole.LEADER,
                LocalDateTime.of(2023, 7, 19, 12, 0, 0), savedGoalRoom, member1.getId());
        final GoalRoomMember goalRoomMember2 = new GoalRoomMember(GoalRoomRole.FOLLOWER,
                LocalDateTime.of(2023, 7, 20, 12, 0, 0), savedGoalRoom, member2.getId());
        final GoalRoomMember goalRoomMember3 = new GoalRoomMember(GoalRoomRole.FOLLOWER,
                LocalDateTime.of(2023, 7, 21, 12, 0, 0), savedGoalRoom, member3.getId());
    ...
    void 골룸_아이디와_사용자_아이디로_골룸_멤버를_조회한다() {
        ...
        final GoalRoomMember goalRoomMember = new GoalRoomMember(
                GoalRoomRole.LEADER, LocalDateTime.now(), goalRoom, creator.getId()
        );
        goalRoomMemberRepository.save(goalRoomMember);

        // when
        final GoalRoomMember findGoalRoomMember = goalRoomMemberRepository.findByGoalRoomIdAndMemberId(goalRoom.getId(),
                creator.getId()).get();
