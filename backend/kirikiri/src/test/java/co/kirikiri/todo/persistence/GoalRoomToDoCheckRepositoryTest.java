...
class GoalRoomToDoCheckRepositoryTest {
    ...
    void 골룸_아이디와_투두_아이디와_사용자_아이디로_골룸_투두_체크_현황을_반환한다() {
        ...
        final GoalRoomToDoCheck firstGoalRoomToDoCheck = new GoalRoomToDoCheck(savedGoalRoomMember.getId(),
                savedFirstGoalRoomToDo);
        final GoalRoomToDoCheck secondGoalRoomToDoCheck = new GoalRoomToDoCheck(savedGoalRoomMember.getId(),
                savedSecondGoalRoomToDo);
    ...
    void 골룸_아이디와_사용자_아이디로_골룸_투두_체크_현황을_반환한다() {
        ...
        // when
        final List<GoalRoomToDoCheck> findGoalRoomTodoCheck = goalRoomToDoCheckRepository.findByGoalRoomIdAndGoalRoomMemberId(
                savedGoalRoom.getId(), savedGoalRoomMember.getId());
    ...
    @Test
    void 골룸_멤버와_투두_아이디로_골룸_투두_체크를_삭제한다() {
        // given
        final Member creator = 사용자를_생성한다("name1", "01011111111", "identifier1", "password!1");
        final RoadmapCategory category = 카테고리를_저장한다("여가");
        final RoadmapNode roadmapNode1 = 로드맵_노드를_생성한다("로드맵 1주차", "로드맵 1주차 내용");
        ...
        // then
        final List<GoalRoomToDoCheck> findGoalRoomToDoChecks = goalRoomToDoCheckRepository.findByGoalRoomIdAndGoalRoomMemberId(
                savedGoalRoom.getId(), savedGoalRoomMember.getId());
        assertThat(findGoalRoomToDoChecks)
                .usingRecursiveComparison()
                .ignoringFields("id")
                .isEqualTo(List.of(secondGoalRoomToDoCheck));
    }
